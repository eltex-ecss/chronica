...
%%%-------------------------------------------------------------------
-module(chronica_disk_log_server).
...
%%%-----------------------------------------------------------------
%%% This module implements the chronica_disk_log server.  Its primary purpose
%%% is to keep the ets table 'chronica_disk_log_names' updated and to handle
...
%%%----------------------------------------------------------------------
start_link() ->
    gen_server:start_link({local, chronica_disk_log_server}, chronica_disk_log_server, [], []).
...
open({ok, A}) ->
    ensure_started(),
    gen_server:call(chronica_disk_log_server, {open, local, A}, infinity);
open(Other) ->
    Other.
...
dist_open(A) ->
    ensure_started(),
    gen_server:call(chronica_disk_log_server, {open, distr, A}, infinity).

close(Pid) ->
    gen_server:call(chronica_disk_log_server, {close, Pid}, infinity).
...
%% like {distributed_chronica_disk_log, Log}, but backward compatibility makes
...
ensure_started() ->
    case whereis(chronica_disk_log_server) of
    undefined ->
        LogSup = {chronica_disk_log_sup, {chronica_disk_log_sup, start_link, []}, permanent,
              1000, supervisor, [chronica_disk_log_sup]},
        supervisor:start_child(kernel_safe_sup, LogSup),
        LogServer = {chronica_disk_log_server,
             {chronica_disk_log_server, start_link, []},
             permanent, 2000, worker, [chronica_disk_log_server]},
        supervisor:start_child(kernel_safe_sup, LogServer),
        Police = {chronica_disk_log_police,
             {chronica_disk_log_police, start, [?DEFAULT_FILE_CHECK_TIMEOUT]},
             permanent, 2000, worker, [chronica_disk_log_police]},
        supervisor:start_child(kernel_safe_sup, Police),
        ok;
    _ -> ok
    end.
...
%% chronica_disk_log_servers mutually open chronica_disk_logs.
...
start_log(Name, Req, From, State) ->
    Server = self(),
    case supervisor:start_child(chronica_disk_log_sup, [Server]) of
    {ok, Pid} ->
...
do_get_log_pids(LogName) ->
    ...
    [{LogName, _Pid, distr}] ->
            case pg2:get_members(?group(LogName)) of
                [] -> % The chronica_disk_log process has died recently
                    undefined;
                Members ->
                    {distributed, Members}
            end;
