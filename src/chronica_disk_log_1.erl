...
repair(In, File) ->
    FSz = file_size(File),
    error_logger:info_msg("chronica_disk_log: repairing ~p ...\n", [File]),
...
mf_int_chunk(Handle, {FileNo, Pos}, Bin, N) ->
    ...
    {error, _Reason} ->
       error_logger:info_msg("chronica_disk_log: chunk error. File ~p missing.\n\n",
                 [FName]),
...
mf_ext_inc(Handle, Head) ->
    %?INT_DBG("mf_ext_inc(~p, ~p)", [Handle, Head]),
    #handle{filename = FName, cur_cnt = CurCnt, cur_name = _FileName,
        acc_cnt = AccCnt, curF = CurF, maxF = MaxF, cur_fdc = CurFdC,
        noFull = _NoFull} = Handle,
    case mf_ext_rotate(FName, CurF, MaxF, CurFdC, Head) of
        {ok, NewCurF, NewFdC, Lost, {Nh, FirstPos}} ->
            ...
            {ok, Handle1, Lost};
        Error ->
            %?INT_ERR("mf_ext_inc(~p, ~p) Error: ~p", [Handle, Head, Error]),
            {error, Error, Handle}
    end.

mf_ext_rotate(FName, CurFNumber, MaxFNumber, CurFdC, Head) ->
    %?INT_DBG("mv_ext_rotate ~p ~p ~p", [FName, CurFNumber, MaxFNumber]),
    NormaizedMaxFNumber = case MaxFNumber of
                            {_, _} -> MaxFNumber;
                            _ -> {MaxFNumber, MaxFNumber}
                          end,
    StartValue = case MaxFNumber of
                    {NewMaxF, OldMaxF} when NewMaxF >= OldMaxF -> NewMaxF;
                    {_NewMaxF, OldMaxF} -> OldMaxF;
                    _ -> MaxFNumber
                 end,
    case catch fclose(CurFdC, FName) of
        ok ->
            case mv_ext_files(FName, StartValue, NormaizedMaxFNumber) of
                ok ->
                    {ok, NewFdC, _NewFileName, Lost, {Nh, FirstPos}, _FileSize} =
                        ext_file_open(FName, 1, 0, 0, Head),
                    NewCurFNumber = case CurFNumber + 1 >= element(1, NormaizedMaxFNumber) of
                                        true -> element(1, NormaizedMaxFNumber);
                                        false -> CurFNumber + 1
                                    end,
                    {ok, NewCurFNumber, NewFdC, Lost, {Nh, FirstPos}};
                Error ->
                    %?INT_ERR("mv_ext_rotate(~p, ~p, ~p, ~p, ~p) error: ~p", [FName, CurFNumber, MaxFNumber, CurFdC, Head, Error]),
                    Error
            end;
        Error ->
            %?INT_ERR("mv_ext_rotate(~p, ~p, ~p, ~p, ~p) error: ~p", [FName, CurFNumber, MaxFNumber, CurFdC, Head, Error]),
...
mv_ext_files(FName, CurFNumber, {NewMaxFNumber, MaxFNumber}) ->
    %?INT_DBG("mv_ext_files ~p ~p ~p", [FName, CurFNumber, MaxFNumber]),
    if
        NewMaxFNumber >= MaxFNumber ->
            case CurFNumber of
                0 -> ok;
                MaxFNumber ->
                    ?INT_DBG("deleting file ~s... ", [add_ext(FName, CurFNumber)]),
                    DelName = add_ext(FName, CurFNumber),
                    case file:delete(DelName) of
                        ok ->
                            %?INT_DBG("ok", []),
                            mv_ext_files(FName, CurFNumber - 1, {NewMaxFNumber, MaxFNumber});
                        {error, enoent} ->
                            %?INT_DBG("not found", []),
                            mv_ext_files(FName, CurFNumber - 1, {NewMaxFNumber, MaxFNumber});
                        Error ->
                            ?INT_ERR("cant delete file ~s cause: ~p", [add_ext(FName, CurFNumber), Error]),
                            Error
                    end;
                N when N < MaxFNumber ->
                    FromFile = case CurFNumber of
                                1 -> FName;
                                _ -> add_ext(FName, CurFNumber - 1)
                               end,
                    ToFile = add_ext(FName, CurFNumber),
                    %?INT_DBG("renaming file ~s->~s... ", [FromFile, ToFile]),
                    case file:rename(FromFile, ToFile) of
                        ok ->
                            %?INT_DBG("ok", []),
                            mv_ext_files(FName, CurFNumber - 1, {NewMaxFNumber, MaxFNumber});
                        {error, enoent} ->
                            ...
                            mv_ext_files(FName, CurFNumber - 1, {NewMaxFNumber, MaxFNumber});
                        Err ->
                            %?INT_ERR("cant rename file ~s->~s cause: ~p", [FromFile, ToFile, Err]),
                            Err
                    end
                end;
        NewMaxFNumber < MaxFNumber ->
            if
                CurFNumber > NewMaxFNumber ->
                    ?INT_DBG("deleting file ~s... ", [add_ext(FName, CurFNumber)]),
                    DelName = add_ext(FName, CurFNumber),
                    case file:delete(DelName) of
                        ok -> mv_ext_files(FName, CurFNumber - 1, {NewMaxFNumber, MaxFNumber - 1});
                        {error, enoent} -> mv_ext_files(FName, CurFNumber - 1, {NewMaxFNumber, MaxFNumber - 1});
                        Err -> Err
                    end
            end
    end.
...
%% -> {NewNumber, MaxNumber, {new_file_descriptor}, Filename.2, _, _, _}
...
-define(index_file_name(F), make_index_file_name(F)).

make_index_file_name(F) ->
    filename:dirname(F) ++ "/.disk_log/"  ++ filename:basename(add_ext(F, "idx")).
...
-define(size_file_name(F), make_size_file_name(F)).

make_size_file_name(F) ->
    %io:format("~s~n", [?FUNC_STRING]),
    filename:dirname(F) ++ "/.disk_log/"  ++ filename:basename(add_ext(F, "siz")).
...
read_size_file_version(FName) ->
    SizeFileName = ?size_file_name(FName),
    filelib:ensure_dir(SizeFileName),
    case file:read_file(SizeFileName) of
