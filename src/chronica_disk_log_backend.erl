...
%%% @end
%%%-------------------------------------------------------------------
-module(chronica_disk_log_backend).
...
handle_write(Handler, Data, TypeFormat) when TypeFormat =:= binary ->
    try
        case disk_log:balog(Handler, chronica_format_creator:frame(Data)) of
            {error, Reason} ->
                {error, Reason};
            ok ->
                ok
...
handle_write(Handler, Str, _TypeFormat) ->
    try
        case disk_log:balog(Handler, Str) of
            {error, Reason} ->
                {error, Reason};
            ok -> ok
        end
...
handle_rotate(Handle) ->
    disk_log:inc_wrap_file(Handle).

handle_check(Handle) ->
    case disk_log:info(Handle) of
        {error, no_such_log} ->
            false;
        {error, einval} ->
            false;
...
check_filename(Name, LogRoot) ->
    ...
    Checked = normalize(LogRoot ++ "/" ++ TmpName),
    case filelib:ensure_dir(Checked) of
        ok ->
